...
# Global caches for performance (avoid repeated API calls)
_cached_agent_ids: dict[str, str] | None = None


def clear_agent_cache() -> None:
    """Clear the agent cache to force re-fetching from Foundry."""
    global _agent_cache, _cached_agent_ids
    _agent_cache = {}
    _cached_agent_ids = None


def get_cached_agent_ids() -> dict[str, str]:
    """Get agent IDs with caching to avoid repeated file reads."""
    global _cached_agent_ids
    if _cached_agent_ids is None:
        if os.path.exists(AGENT_IDS_FILE):
            with open(AGENT_IDS_FILE, 'r') as f:
                _cached_agent_ids = json.load(f)
        else:
            _cached_agent_ids = {}
    return _cached_agent_ids
...
def get_agent_configs():
    ...
    - Agents without external tools use the mini model for faster inference
    - Agents with tools (search, Bing) use the full model for better reasoning
    """
    return {
        "DocumentConsistencyAgent": {
            "yaml": "document-consistency-agent.yaml",
            "local_tools": [],
            "model": AZURE_AI_MODEL_MINI_DEPLOYMENT_NAME,  # No tools, use mini
        },
        "HSCodeValidationAgent": {
            ...
            "model": AZURE_AI_MODEL_DEPLOYMENT_NAME,  # Has AI Search tools
        },
        ...
        "ComplianceAggregatorAgent": {
            ...
            "local_tools": [],
            "model": AZURE_AI_MODEL_MINI_DEPLOYMENT_NAME,  # No tools, aggregation is simple
        },
    }
...
# Global agent cache to avoid repeated Foundry API calls
...
class FoundryAgentExecutor(Executor):
    ...
    Uses caching to avoid repeated API calls to Foundry.
    """
    ...
    def __init__(
        ...
        id: str | None = None,
        cached_agent: Any = None,  # Pre-fetched agent for faster execution
    ):
        ...
        self.tools = tools or []
        self._agent = cached_agent  # Use pre-cached agent if provided
    ...
    async def handle(self, declaration: dict[str, Any], ctx: WorkflowContext[AgentResult]) -> None:
        ...
        try:
            # Use cached agent or fetch from Foundry (with global caching)
            if self._agent is None:
                async with _agent_cache_lock:
                    if self.agent_name in _agent_cache:
                        self._agent = _agent_cache[self.agent_name]
                    else:
                        self._agent = await self.provider.get_agent(name=self.agent_name)
                        _agent_cache[self.agent_name] = self._agent
...
async def run_compliance_check(
    ...
    # Get or create agent IDs (use cache for performance)
    if agent_ids is None:
        agent_ids = get_cached_agent_ids()
        if not agent_ids:
    ...
    async with (
        ...
        # Pre-fetch all agents in parallel for faster workflow execution
        agent_configs = get_agent_configs()
        agent_names_to_fetch = [name for name in agent_configs.keys() if name in agent_ids]
        
        print(f"Pre-fetching {len(agent_names_to_fetch)} agents from Foundry...")
        fetch_start = __import__('time').time()
        
        # Parallel fetch of all agents (major performance improvement)
        async def fetch_agent(name: str):
            try:
                return name, await provider.get_agent(name=name)
            except Exception as e:
                print(f"  Warning: Could not pre-fetch {name}: {e}")
                return name, None
        
        fetch_results = await asyncio.gather(
            *[fetch_agent(name) for name in agent_names_to_fetch],
        ...
        for result in fetch_results:
            if isinstance(result, tuple):
                ...
                if agent is not None:
                    cached_agents[name] = agent
        
        fetch_time = int((__import__('time').time() - fetch_start) * 1000)
        print(f"  ✓ Pre-fetched {len(cached_agents)} agents in {fetch_time}ms")
        
        # Create executor for each Foundry agent with pre-cached agents
        agent_executors = []
        for name, config in agent_configs.items():
            ...
            executor = FoundryAgentExecutor(
                ...
                cached_agent=cached_agents.get(name),  # Pass pre-cached agent
            )
