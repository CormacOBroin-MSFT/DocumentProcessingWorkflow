...
def get_agent_configs():
    ...
    All agents use gpt-41-mini for consistent, cost-effective inference.
    """
    return {
...
async def create_foundry_agents(delete_existing: bool = False) -> dict[str, str]:
    ...
    async with (
        ...
        for name, config in agent_configs.items():
            ...
            agent_ids[name] = created_agent.name
            print(f"  ✓ Created: {name} (ID: {created_agent.id})")
...
async def run_declarative_workflow(declaration: dict[str, Any], trace: bool = False) -> dict[str, Any]:
    ...
    workflow_path = Path(__file__).parent / "compliance-workflow.yaml"
...
async def main():
    ...
    if args.declarative:
        ...
        print("=" * 70)
        print()
        print("Using YAML-defined workflow from: compliance-workflow.yaml")
